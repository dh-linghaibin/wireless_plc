...
//    SPI_InitStructure.SPI_CRCPolynomial = 7;    //CRC值计算的多项式
    ...
    SPI_InitStructure.SPI_CPOL = SPI_CPOL_Low;//????á?′?DDê±?óμ??èì?:ê±?óDü????
    SPI_InitStructure.SPI_CPHA = SPI_CPHA_1Edge;//êy?Y2???óúμú?t??ê±?ó??
    SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;	 //NSSD?o?óéó2?t￡¨NSS1ü??￡??1ê?èí?t￡¨ê1ó?SSI??￡?1üàí:?ú2?NSSD?o?óDSSI??????
    SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_16;//?¨ò?2¨ì??ê?¤·??μμ??μ:2¨ì??ê?¤·??μ?μ?a2
