...
#define INA_INSIDE_IIC    //使用自带的io模拟iic
        ...
        #define IIC_RCC         RCC_AHB1Periph_GPIOB                                                //端口时钟
        #define IIC_SDA         GPIO_Pin_11                                                                        //SDA引脚
        #define IIC_SDA_H       GPIO_SetBits(IIC_PORT,IIC_SDA)                            //拉高SDA
        #define IIC_SDA_L       GPIO_ResetBits(IIC_PORT,IIC_SDA)                        //拉低SDA
        #define IIC_SDA_READ()  GPIO_ReadInputDataBit(IIC_PORT,IIC_SDA)    //读取SDA

        #define IIC_SCL         GPIO_Pin_10                                                                        //SCL引脚
        #define IIC_SCL_H       GPIO_SetBits(IIC_PORT,IIC_SCL)                            //拉高SCL
        #define IIC_SCL_L       GPIO_ResetBits(IIC_PORT,IIC_SCL)                        //拉低SCL
#endif

void ina219_init(void );
unsigned int ina219_get_voltage_mv(void);//获取电压（单位：mv）
unsigned int ina219_get_current_ma(void);//获取电流（单位：mA）
unsigned int ina219_get_power_mv(void);    //获取当前功率（单位：mw）
